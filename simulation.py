...
# Координаты изображения сигнала, таймер и количество транспортных средств
...
stoppingGap = 25    # остановочный зазор
movingGap = 25   # движущийся зазор

# установить разрешенные типы транспортных средств здесь
...
# установите здесь случайное время или время зеленого сигнала по умолчанию
randomGreenSignalTimer = True
# установить случайный диапазон времени зеленого сигнала здесь
...
# Указание цвета сигнала светофора
class TrafficSignal:
...
# Указание свойств транспорта и описание движения
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        # Установить новую начальную и конечную координаты
        if(direction=='right'):
...
# Распечатать таймеры сигналов на cmd
def printStatus():
    for i in range(0, 4):
        if(signals[i] != None):
            if(i==currentGreen):
...
def repeat():
    ...
    # сбросить все времена сигнала текущего сигнала к значениям по умолчанию/случайным временам
    if(randomGreenSignalTimer):
    ...
    signals[nextGreen].red = signals[currentGreen].yellow+signals[currentGreen].green    # установите время красного цвета рядом со следующим сигналом как (время желтого + время зеленого) следующего сигнала
    repeat()

# Обновлять значения сигнальных таймеров каждую секунду
def updateValues():
...
# Генерация транспортных средств в симуляции
...
# Указание общего времени в симуляции
def simTime():
...
# Обобщение и задача визуальных состовляющих
class Main:
    ...
    thread2 = threading.Thread(name="generateVehicles",target=generateVehicles, args=())    # Генерация транспортных средств
    thread2.daemon = True
    thread2.start()
    ...
    while True:
        ...
        screen.blit(background,(0,0))   # отображать фон в симуляции
        for i in range(0,noOfSignals):  # отображать сигнал и устанавливать таймер в соответствии с текущим состоянием: зеленый, желтый или красный
            if(i==currentGreen):
                if(currentYellow==1):
        ...
        # таймер сигнала дисплея
        for i in range(0,noOfSignals):  
            signalTexts[i] = font.render(str(signals[i].signalText), True, white, black)
            screen.blit(signalTexts[i],signalTimerCoods[i])

        # отображать количество транспортных средств
        for i in range(0,noOfSignals):
            displayText = vehicles[directionNumbers[i]]['crossed']
            vehicleCountTexts[i] = font.render(str(displayText), True, black, white)
            screen.blit(vehicleCountTexts[i],vehicleCountCoods[i])

        # прошедшее время отображения
        timeElapsedText = font.render(("Time Elapsed: "+str(timeElapsed)), True, black, white)
        screen.blit(timeElapsedText,timeElapsedCoods)

        # отображать транспортные средства
